...
Page({
  ...
  dataProcessing(schools: Array<object>) {
    ...
    regions.forEach((region) => {
        countriesInRegions[region] = schoolsInRegions[region].reduce((prev: Array<any>, cur: any) => {
    ...
    regions.forEach((region) => {
        ...
        countriesInRegions[region].forEach((country: any) => {
            filteredSchools[region][country.value] =  schoolsInRegions[region].filter((school: any) => school['country'] == country.value);
            console.log(filteredSchools);
        });
    });
    return { regions, schoolsInRegions, countriesInRegions, selectedCountry, filteredSchools };
  },
  onLoad: function (_options: any) {
    const schools = wx.getStorageSync('data');
    const { regions, schoolsInRegions, countriesInRegions, selectedCountry, filteredSchools } = this.dataProcessing(schools);
    this.setData({
      regions: regions,
